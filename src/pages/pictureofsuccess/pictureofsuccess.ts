...
@IonicPage()
@Component({
  selector: 'page-pictureofsuccess',
  templateUrl: 'pictureofsuccess.html',
})
export class PictureofsuccessPage {
  store_id: any;
  store_name: any;
  rep_id: any;
  rep_name: any;
  ...
  img_count = 0;
	...
	constructor (public http: Http, private nav: NavController, private auth: AuthService, private loadingCtrl: LoadingController, private alertCtrl: AlertController, private navParams: NavParams, private geolocation: Geolocation, private camera: Camera, private transfer: Transfer, private file: File, private filePath: FilePath, public actionSheet: ActionSheetController, public toastCtrl: ToastController, public platform: Platform) {
		...
		this.get_pos_categories();
  	}

	  ionViewDidLoad() {
	    console.log('ionViewDidLoad PictureofsuccessPage');
	  }

	public get_pos_categories() {
		this.auth.showLoading();
		this.http.get(API_URL + '/GetPOSDetails?store_id='+this.store_id).map(res => res.json()).subscribe(data => {
			var response 			= data.data;

			this.pos_categories 	= response;
	...
	// category.poc_id, category.pos_upload_image, category.category_id, category.category_name
	public gotoposfill (poc_id, pos_upload_image, category_id, category_name){
		this.nav.setRoot (PictureofsuccessformPage, {day_of_week: this.day_of_week, store_id: this.store_id, store_name: this.store_name, category_id: category_id, category_name: category_name, poc_id: poc_id, pos_upload_image: pos_upload_image});
	}
